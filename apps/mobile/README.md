...
**When you bump `app.json`'s `version`:** you don't need to touch anything else. The native iOS project is committed to git (not regenerated by EAS build), so `ios/TikiAcca/Supporting/Expo.plist`'s `EXUpdatesRuntimeVersion` would otherwise drift from `app.json` and silently break OTA updates (below) — `scripts/sync-native-version.js` keeps it in sync automatically, run via the `eas-build-post-install` hook (`package.json`) on every EAS build. Run `npm run sync:native-version` locally if you want the committed plist to show the new value immediately instead of waiting for the next build (purely cosmetic — EAS patches it either way before archiving).
