...
export function SettleRoundForm({
  ...
  async function handleAutoSettle() {
    ...
    setAutoMessage("");
    try {
      await api(`/api/rounds/${round.id}/auto-settle`, { method: "POST", token });
      onSettled();
    } catch (e) {
      if (e instanceof ApiError && e.status === 409) {
        setAutoMessage("Some legs are not ready yet — try again after matches finish.");
      }
      setError(e instanceof ApiError ? e.message : "Auto-settle failed");
    } finally {
  ...
  async function handleSubmit() {
    ...
    setError("");
    try {
      await api(`/api/rounds/${round.id}/settle`, {
        method: "POST",
        token,
        body: JSON.stringify({
      ...
      setError(e instanceof ApiError ? e.message : "Failed to settle");
    } finally {
      setLoading(false);
    }
  }

  const outcomesList: LegOutcome[] = ["won", "lost", "void"];

  return (
    <Card>
      <Text style={styles.sectionTitle}>Settle round (owner)</Text>
      <Text style={styles.meta}>
        Auto-settle uses synced match results. Manual settle below if needed.
      </Text>
      <Button
        label={autoLoading ? "Fetching results…" : "Auto-settle from results"}
        onPress={() => void handleAutoSettle()}
        loading={autoLoading}
        variant="secondary"
      />
      {autoMessage ? <Text style={styles.meta}>{autoMessage}</Text> : null}

      {round.legs.map((leg) => (
        <View key={leg.id} style={styles.settleLeg}>
          ...
          <View style={styles.outcomePicker}>
            {outcomesList.map((o) => (
              <Pressable
                key={o}
                onPress={() => setOutcomes((prev) => ({ ...prev, [leg.id]: o }))}
                style={[
                  styles.outcomeOption,
                  (outcomes[leg.id] ?? "lost") === o && styles.outcomeOptionSelected,
                ]}
              >
                <Text
                  style={[
                    styles.outcomeOptionText,
                    (outcomes[leg.id] ?? "lost") === o && styles.outcomeOptionTextSelected,
                  ]}
                >
                  {o}
                </Text>
      ...
      <ErrorText message={error} />
      <Button
        label={loading ? "Settling…" : "Settle round & award points"}
