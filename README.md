...
apps/mobile/       Expo iOS + Android client (native projects generated by expo prebuild)
packages/shared/   Zod schemas, API types, view logic, copy shared by both apps
packages/client/   React data hooks shared by both apps (group data, chat, leg picker)
packages/database/ Prisma schema + client
tools/marketing/   Social + App Store image tooling
scripts/brand/     Brand colour sync check + icon generation
scripts/ops/       Production runbook scripts
...
| `npm run lint` | Brand sync check + ESLint |
| `npm run typecheck` | Type-check web, mobile and shared |
| `npm test` | Web + shared tests (web tests need a migrated `DATABASE_URL`) |
...
- **PR checks:** GitHub Actions `ci.yml` — lint, typecheck, tests against Postgres
