...
| `packages/shared` | Zod schemas, types, constants, display helpers shared by both apps |
...
npm run db:generate
npm run lint             # brand sync check + ESLint (web)
npm run typecheck        # web, mobile, shared
npm test                 # web + shared; web tests need a migrated DATABASE_URL
```
...
- Components are grouped by area: `components/admin/`, `components/group/`, `components/layout/` (web); `src/components/round/` (mobile). Logic both apps need goes in `packages/shared`, not copied into each app
...
- Reuse types and helpers from `@tiki-acca/shared`; match web flows (auth, dashboard, group detail, submit leg)
