...
8. Each round has a web/mobile **Banter** thread. Pick submissions, changes, lock, results, and settlement appear as system messages; members can post text and react using six one-tap defaults or any single emoji through `+`. Pick rows mirror reactions from their latest announcement.
