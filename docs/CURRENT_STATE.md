...
**Navigation:** `AppNav` in header (Groups ↔ Performance ↔ Admin for platform admins ↔ Notifications ↔ About). Logo links to `/` (marketing home). Signed-in visits to `/` and `/about` use `AppHeader` instead of the marketing sign-in/up header. Inside a group, `GroupNav` tabs (Round / History / Leaderboard / Performance) share data via `GroupDataProvider` (fetched once in group layout; polls every 60s while acca locked).
