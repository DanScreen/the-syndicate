...
- **Unit / integration** — `npm test`: `*.test.ts` next to the code (node:test). Web tests need a migrated `DATABASE_URL`.
- **End-to-end** — `npm run test:e2e` (Playwright, `apps/web/e2e/`, config `apps/web/playwright.config.ts`). Needs Postgres from `docker compose up -d` and a one-off `npx playwright install chromium` (from `apps/web`). It builds the web app (`next build && next start` on port 3100). Stop `npm run dev` first, because both write `apps/web/.next`. Global setup migrates and **wipes** a dedicated database (`tiki_acca_e2e`; override with `E2E_DATABASE_URL`, which must contain `e2e` or `test`; the port can be changed with `E2E_PORT`). It then seeds a World Cup odds snapshot and an admin (`e2e-admin@tikiacca.com`). The server runs with `ODDS_DB_ONLY` and one bookmaker per selection, so prices and points are deterministic and nothing calls The Odds API. Email, push and results feeds are blanked. Each browser context sends its own `x-forwarded-for`, so the per-IP auth rate limits don't trip.
  - **Specs:**
    - `golden-path`: sign-up → email gate → create group → invite → friend signs up from the link and verifies in another tab → both pick → lock → admin settles → dashboards and leaderboard. It also runs at Pixel 7 size.
    - `solo-acca`: multi-leg, remove, manual lock.
    - `settlement`: one losing leg.
    - `auth`: deep link survives sign-in, bad password, duplicate sign-up, email gate, verify links.
    - `group-access`: join by code, bad code, non-members bounced.
    - `chat`: post, profanity blocked.
    - `api-guards`: 401 and 403 on member, admin and cron routes.
  - **Writing specs:** each spec creates its own users and groups (`e2e/support/db.ts`), so they run in parallel. Settled outcomes propagate to every pending leg on the same selection (see [Settlement](#settlement)), so a spec that settles must use fixtures no other spec picks (`e2e/support/fixtures-data.ts`). For the same reason, `--repeat-each` makes settling specs collide.
  - **Debugging:** a failure writes a trace to `apps/web/test-results/`; open it with `npx playwright show-trace <trace.zip>`.

### Deploy

PRs and pushes to `main` → `.github/workflows/ci.yml`: lint (web, mobile, `packages/client`), typecheck, tests against Postgres, and a separate `e2e` job (Playwright against a production build; report + traces uploaded on failure).
...
| Shared view logic & copy | `packages/shared/src/round-view.ts` (`deriveRoundView` — everything the Bet tab shows; `accaSummaryCopy`), `stats-display.ts` (`filterUserStatsByGroup`, `buildShareText`, chart labels), `copy.ts` (`copy`, `COMPLIANCE` gambling wording + helpline: signpost `GambleAware.org` → `https://www.gambleaware.org`, commission stated as "We earn commission from some bookmakers…"; `NOTIFICATION_PREFERENCE_SECTIONS`) |
...
**Void legs count at 1.00.** `effectiveAccaOdds()` (`packages/shared/src/scoring.ts`) drops void legs from the acca price at read time, as bookmakers do: an acca locked at 4.00 whose 2.00 leg is voided pays at 2.00, and one with every leg void returns the stake (P/L 0). `Round.combinedOdds` is never overwritten; `lockedPriceIncludesVoidLegs()` tells a price captured before the void (recompute from live legs) from a reprice after a swap (already excludes it). Settlement P/L and group points, the settle chat message, history, stats, active bet summaries and the betslip view all use it. When the stored price still includes a void leg the round view drops the lock-time bookmaker rankings, since those prices are stale.
...
| Method | Route | Notes |
|--------|-------|-------|
| Auto (hands-off) | Via `POST /api/internal/sync-matches` | Cron sync → write leg outcomes provisionally at FT → settle locked rounds once FT scores are stable (15 min unchanged, max 4h) when any confirmed leg loses **or** all legs are confirmed won/void; reconcile feed score corrections for 24h after FT; continue resolving pending legs on early-settled losses |
...
**FT confirmation window.** When match sync first observes `FINISHED`, it stamps `Match.finishedAt` and `Match.scoreStableSince`. Leg outcomes are written **provisionally at the first FT reading** (members see won/lost within one cron run), but a round only **settles** from a result once the FT score has been **unchanged for** `RESULT_CONFIRMATION_MS` (15 minutes, `packages/shared/src/constants.ts`; 1h before 24 Sep 2026), capped at `RESULT_CONFIRMATION_MAX_MS` (4 hours) from first FINISHED — so feeds can correct provisional scores (disallowed goals / VAR) before the acca is busted or won. A settled round is never reopened, which is why settlement (not the leg result) waits: `resolveRoundOutcomes()` returns unconfirmed legs as `provisional`, and `tryAutoSettleRound()` treats them as pending when deciding whether the round is settleable. Each feed score change resets `scoreStableSince` and restarts the 15-minute clock. During the wait the Match row keeps updating to the latest feed score. **After outcomes are written**, the same cron keeps reconciling for `RESULT_RECONCILE_MS` (24 hours): if the feed score later disagrees with a leg’s won/lost/void, outcomes and points are auto-corrected via `reconcileMatchLegOutcomes()` (chat correction + P/L delta) — no admin required. On a still-locked round `correctLegOutcome()` only moves the outcome (no points or won/lost counters — `applyRoundSettlement()` awards every leg in full at settle), so a corrected provisional result is never double-counted. Covered by `apps/web/src/lib/settlement/auto-settle-round.test.ts` (requires local PostgreSQL). Admin score overrides set `scoreLocked` and confirm immediately. Existing FINISHED rows are backfilled so a deploy does not re-open the window.

Email and push notifications fire on **round locked**, **round settled**, **pick reminders** (within 2h before first kickoff) and **pick voided** (your pick's match was postponed or cancelled — see [Void picks](#void-picks-postponed-or-cancelled-matches); governed by the pick-reminder preference). Resend for email (`RESEND_API_KEY`, `EMAIL_FROM`); Expo Push API for mobile (`PushDevice` tokens). Per-user preferences at `/account` (web) and `(main)/account` (mobile). Deduped via `NotificationLog`; round-level `lockedNotificationSentAt` / `settledNotificationSentAt` set only when all members are satisfied (delivered or opted out). Failed lock/settle deliveries retried on `sync-matches` (5 min). Pick reminders cron: `POST /api/internal/round-reminders` every 15 min (Terraform). Notification times formatted in `Europe/London`. Emails only go to confirmed addresses (see [Email verification](#email-verification)). See [specs/notifications.md](./specs/notifications.md).
...
- `PATCH` returns `{ leg, repriced, locked }`; `locked` is true when the change completed an open round and locked it (a swap on a reopened round).

### Void picks (postponed or cancelled matches)

`voidPostponedLegs()` (`apps/web/src/lib/rounds/void-postponed-legs.ts`) runs on every `sync-matches` cron, after reconcile and before the kickoff lock. For each pending leg on an open or locked round whose `Match` is `POSTPONED`, `CANCELLED`, `SUSPENDED` or `AWARDED` (`isVoidMatchStatus()`, unless held for admin review) it claims `pending → void` and posts a `leg_result` chat message. Then per round:

- **Locked, other legs not yet kicked off:** the round **reopens** (`locked → open`, `Round.reopenedAt` set, group status back to `open`, `lockedNotificationSentAt` cleared) and a `round_reopened` chat message names the deadline. The deadline is the first kickoff among the **non-void** legs (`firstKickoff()` skips void legs everywhere, so the kickoff lock uses it too).
- **Locked, a live leg has kicked off:** stays locked; the acca rides on without the void leg at the lower price.
- **Open:** stays open; the owner can swap the pick until the first kickoff like any change.

The owner of each void leg gets a `pick_voided` email and push (deduped per leg), saying whether and until when they can swap it, and the acca's price without it.

A **reopened** round only accepts swaps: `POST /api/legs` returns 409 and `DELETE /api/legs/[id]` returns 403. The owner swaps with `PATCH /api/legs/[id]` (UI: **Swap** on web, **Swap void pick** on mobile, plus a warning banner from `deriveRoundView().voidBanner` for every member). The round relocks and reprices as soon as no void legs remain (`openRoundReadyToLock()` in `packages/shared/src/legs-quota.ts`), or at the deadline via the normal kickoff lock, which then prices only the live legs (`claimAndLockRound` excludes void legs).

**Called-off matches can't be picked.** The odds cache keeps listing a postponed match at its original kickoff, so `calledOffFixtureIds()` (`apps/web/src/lib/results/called-off-fixtures.ts`) matches fixtures to `Match` rows (by Odds API id, or teams on the kickoff day) with a void status. `GET /api/fixtures` hides them, and `POST /api/legs` / `PATCH /api/legs/[id]` refuse them with 409, including swapping a void pick to another market on its own postponed match (the one-pick-per-fixture check skips the pick being edited). Otherwise the new pick would go void again after the acca was priced with it. `PATCH` also returns 403 on a locked acca whose picks are all void (it settles at 1.00), and a reprice that fails restores the old pick's outcome as well as the pick.

Void is **sticky**: settlement and reconcile keep a void leg void when its match is later rescheduled for another day (`resolveRoundOutcomes`, `reconcileMatchLegOutcomes`). Postponements are only seen as fast as the feeds report them: football-data.org covers open rounds, while API-Football (League One/Two) polls only locked rounds from 10 minutes before kickoff. Covered by `void-postponed-legs.test.ts` and `called-off-fixtures.test.ts` (require local PostgreSQL), `packages/shared/src/scoring.test.ts` and the void tests in `round-view.test.ts` / `legs-quota.test.ts`.

### Cron (internal)

| Route | Role |
|-------|------|
| `POST /api/internal/sync-matches` | Link locked legs to `Match` → football-data.org + API-Football observations → consensus `Match` result; reconcile recent FT score corrections → leg outcomes; voids picks on postponed/cancelled matches and reopens locked rounds for a swap (`voidedLegs` in the response); locks open rounds at first kickoff; auto-settles locked rounds (incl. early loss); awards deferred legs on settled rounds; retries pending lock/settle notifications |
...
- `Round.reopenedAt` — set when a locked round reopens so a void pick can be swapped (migration `20260926120000_round_reopened_at`); a reopened round only accepts swaps.
...
| `GET /api/fixtures` | Session | List fixtures (`?competition=` required); postponed/cancelled matches hidden |
| `GET /api/fixtures/[id]/markets` | Session | Extended markets (`?competition=` required) |
| `POST /api/legs` | Session | Submit leg (rejects any second leg on the same fixture — 409; 409 on a reopened round or a postponed/cancelled match) |
| `PATCH /api/legs/[id]` | Leg owner | Edit own pick until first kickoff (locked rounds reprice; one-leg-per-fixture rule); swapping the last void pick relocks a reopened round (`locked: true`); 409 for a postponed/cancelled match |
| `DELETE /api/legs/[id]` | Leg owner | Remove own pick while round is open and before first kickoff (403 on a reopened round) |
...
10. **Mobile app** — Native app code complete, feature parity across iOS/Android (single codebase, no platform forks). **iOS live in App Store Connect** (submitted, build 5). **Android** built (Firebase push wired up) but not being released on Play (gambling policy; Android users use the website), see [ANDROID_LAUNCH.md](../apps/mobile/ANDROID_LAUNCH.md). Dev testing: Expo Go or `expo run:ios --device` ([DEVELOPER_TESTING.md](../apps/mobile/DEVELOPER_TESTING.md)); friend distribution via [FRIEND_TESTING.md](../apps/mobile/FRIEND_TESTING.md). Leg-edit parity shipped (same "Change my pick" flow as web). Admin pages are web-only by design. Web and mobile share their data layer (`@tiki-acca/client`), Bet-tab view logic (`deriveRoundView`) and copy — see [specs/mobile-apps.md](./specs/mobile-apps.md#reducing-duplicated-effort-and-artifacts).
11. **Auth JWT** — middleware uses edge-safe `auth.config.ts` (no Prisma); `auth.ts` refreshes `role`, `email` and `isEmailVerified` from DB on each session update. Middleware reads the cookie's copy of `isEmailVerified` and never re-issues the cookie (see [Email verification](#email-verification)), so a verification done elsewhere takes effect on the next `/api/auth/session` fetch. The verify page calls `update()`, which also broadcasts to the user's other tabs.
