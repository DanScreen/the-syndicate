...
| `apps/web/src/components/app-nav.tsx` | Header nav: Home / About / Groups / Performance / Admin / Notifications / Blog |
...
**Navigation:** Logo + **Social Group Betting** tagline always shown. Logo and **Home** → `/`. `AppNav` order: Home → About → Groups → Performance → Admin (admins) → Notifications → **Blog** (rightmost). Signed-in marketing pages use `AppHeader`; signed-out use `MarketingHeader` (Home / About / Sign in / Sign up / Blog). Inside a group, `GroupNav` tabs (Round / History / Leaderboard / Performance) share data via `GroupDataProvider` (fetched once in group layout; polls every 60s while acca locked).
