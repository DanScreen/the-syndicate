...
Nineteen competitions in `packages/shared/src/competitions.ts`: EPL, Championship, League One, League Two, La Liga, Ligue 1, Serie A, Bundesliga, Eredivisie, Primeira Liga, Brazil Série A, Champions League, European Championship, Copa Libertadores, World Cup, Champions League Qualification, Europa League, Carabao Cup (EFL Cup), and UEFA Nations League. **Admin toggles** which are visible in the leg picker (`CompetitionSetting` table; `/admin/competitions`). Default: **World Cup only**; newly discovered catalogue rows are created disabled. Match sync fetches enabled competitions plus any disabled competition that still has a pending leg, skipping `manualSettlement` competitions (League One, League Two, CL Qualification, Europa League, Carabao Cup, Nations League — not on football-data.org free tier).
