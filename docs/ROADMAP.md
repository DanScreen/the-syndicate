...
| 14 | **Solo accas — finish verification** | Code | Feature shipped (phases 1–3, [spec](./specs/solo-unlimited-legs.md)). Remaining: one manual end-to-end solo run on mobile. Lock-route guard tests were done 2026-09-23. A web browser run is automated as of 2026-09-24 (`apps/web/e2e/solo-acca.spec.ts`); it found and fixed a new group's first acca not opening solo. Solo on-ramp UX (dashboard entry point, invite conversion) deliberately deferred to a later spec |
